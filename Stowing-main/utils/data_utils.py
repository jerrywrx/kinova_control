import copy
import h5py
import numpy as np
# import open3d as o3d as o3d
import os
import random
import sys
import torch
from functools import reduce
import json
from collections import OrderedDict
from json import JSONEncoder

from transforms3d.axangles import axangle2mat
import pdb

class Tee(object):
    def __init__(self, name, mode):
        self.file = open(name, mode)
        self.stdout = sys.stdout
        sys.stdout = self

    def __del__(self):
        sys.stdout = self.stdout
        self.file.close()

    def write(self, data):
        self.file.write(data)
        self.stdout.write(data)

    def flush(self):
        self.file.flush()

    def close(self):
        self.__del__()


def store_data(data_names, data, path):
    with h5py.File(path, 'w') as hf:
        for i in range(len(data_names)):
            if isinstance(data[i], dict):
                # Convert numpy arrays to lists in the dictionary
                data_list = {k: v.tolist() if isinstance(v, np.ndarray) else v for k, v in data[i].items()}
                # Convert dictionary to JSON string
                json_str = json.dumps(data_list)
                # Create dataset and store JSON string
                hf.create_dataset(data_names[i], data=json_str)
            else:
                hf.create_dataset(data_names[i], data=data[i])

def load_data(data_names, path):
    with h5py.File(path, 'r') as hf:
        data = []
        for i in range(len(data_names)):
            d = hf.get(data_names[i])
            # if data_names[i] == 'wall_pos_y' and d ==None: # compatibility with old data
            #     d = hf.get('dis_to_walls')
            # else:
            #     d = hf.get(data_names[i])
            # if d is None:
                # continue
            if d.dtype == np.dtype('object'):
                # JSON string was stored in the dataset, so we load it back as a dictionary
                json_str = d[()].decode('utf-8')
                loaded_dict = json.loads(json_str)
                # Convert lists back to numpy arrays if necessary
                d_np = {k: np.array(v) if isinstance(v, list) else v for k, v in loaded_dict.items()}
                data.append(d_np)
            else:
                data.append(np.array(d))
    return data


class CustomJSONEncoder(JSONEncoder):
    def default(self, obj):
        if isinstance(obj, torch.device):
            return str(obj)
        elif isinstance(obj, np.ndarray):
            return obj.tolist()
        return super(CustomJSONEncoder, self).default(obj)

def store_json(data, file_path):
    # Store the OrderedDict as a JSON file
    # def convert(o):
    #     if isinstance(o, np.ndarray):
    #         return o.tolist()
    #     raise TypeError(f'Object of type {o.__class__.__name__} is not JSON serializable')

    # with open(file_path, 'w') as f:
    #     json.dump(data, f, indent=4, default=convert)
    with open(file_path, 'w') as f:
        json.dump(data, f, indent=4, cls=CustomJSONEncoder)

def numpy_array_hook(obj):
    for key, value in obj.items():
        if isinstance(value, list):
            obj[key] = np.array(value)
    return obj

def load_json(file_path):
    # Load the OrderedDict from the JSON file
    with open(file_path, 'r') as f:
        loaded_dict = json.load(f, 
                                object_pairs_hook=OrderedDict, 
                                object_hook=numpy_array_hook)
    return loaded_dict




def set_seed(seed):
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    np.random.seed(seed)
    random.seed(seed)


def expand(batch_size, info):
    length = len(info.shape)
    if length == 2:
        info = info.expand([batch_size, -1])
    elif length == 3:
        info = info.expand([batch_size, -1, -1])
    elif length == 4:
        info = info.expand([batch_size, -1, -1, -1])
    return info

def str_to_list(s):
    return [float(i) for i in s.split()]

def get_scene_info(data):
    """
    A subset of prepare_input() just to get number of particles
    for initialization of grouping
    """
    positions, shape_quats, scene_params = data
    n_shapes = shape_quats.shape[0]
    count_nodes = positions.shape[0]
    n_particles = count_nodes - n_shapes

    return n_particles, n_shapes, scene_params


def normalize_scene_param(scene_params, param_idx, param_range, norm_range=(-1, 1)):
    normalized = scene_params[:, param_idx]
    low, high = param_range
    if low == high:
        return normalized
    nlow, nhigh = norm_range
    normalized = nlow + (normalized - low) * (nhigh - nlow) / (high - low)
    return normalized


# @profile
def get_env_group(args, B): 
    p_rigid = torch.zeros(B, args.max_n_instance, device=args.device)
    p_rigid[:] = args.p_rigid

    p_instance = torch.zeros(B, sum(args.n_particles_type), args.max_n_instance, device=args.device) 
    particle_count = []
    shelf_particle_sum = 0
    for k,v in args.n_particles_per_object.items():
        if 'shelf' in k:
            shelf_particle_sum += v
        else:
            if shelf_particle_sum > 0:
                particle_count.append(shelf_particle_sum)
            particle_count.append(v)
            
    count = 0
    for i, instance_count in enumerate(particle_count):
        p_instance[:, count:count+instance_count, i] = 1
        count += instance_count
    
    physics_param = torch.zeros(B, sum(args.n_particles_type), device=args.device)
    scene_params = torch.tensor(args.scene_params, device=args.device, dtype=torch.float32)
    scene_params = torch.tile(scene_params, (B, 1))
    norm_g = normalize_scene_param(scene_params, 1, args.physics_param_range)
    physics_param[:] = norm_g.float().view(B, 1)

    # p_rigid: B x n_instance
    # p_instance: B x n_p x n_instance
    # physics_param: B x n_p
    return [p_rigid, p_instance, physics_param]


def count_parameters(model):
    return sum(p.numel() for p in model.parameters() if p.requires_grad)


def get_lr(optimizer):
    for param_group in optimizer.param_groups:
        return param_group['lr']


def batch_normalize(batch, mean, std, eps=1e-10):
    if len(mean.shape) == 1:
        return (batch - mean) / (std + eps)
    elif len(mean.shape) == 2:
        batch_new = []
        for i in range(batch.shape[0]):
            batch_new.append((batch[i] - mean[i]) / (std[i] + eps))
        return torch.stack(batch_new)
    else:
        raise NotImplementedError


def batch_denormalize(batch, mean, std, eps=1e-10):
    if len(mean.shape) == 1:
        return batch * (std + eps) + mean
    elif len(mean.shape) == 2:
        batch_new = []
        for i in range(batch.shape[0]):
            batch_new.append(batch[i] * (std[i] + eps) + mean[i])
        return torch.stack(batch_new)
    else:
        raise NotImplementedError


def get_circle(center, radius, dim, axis, alpha=1.5):
    # sunflower seed arrangement
    # https://stackoverflow.com/a/72226595
    n_exterior = np.round(alpha * np.sqrt(dim)).astype(int)
    n_interior = dim - n_exterior

    k_theta = np.pi * (3 - np.sqrt(5))
    angles = np.linspace(k_theta, k_theta * dim, dim)

    r_interior = np.linspace(0, 1, n_interior)
    r_exterior = np.ones(n_exterior)
    r = radius * np.concatenate((r_interior, r_exterior))

    circle_2d =  r * np.stack((np.cos(angles), np.sin(angles)))
    circle = np.concatenate((circle_2d[:axis], np.zeros((1, dim)), circle_2d[axis:]))
    circle = circle.T + center

    return circle


def get_square(center, unit_size, dim, axis):
    state = []
    n_rows = int(np.sqrt(dim))
    for i in range(dim):
        row = i // n_rows - (n_rows - 1) / 2 # -1, 0, 1
        col = i % n_rows - (n_rows - 1) / 2 # -1, 0, 1
        pos = [unit_size * row, unit_size * col]
        pos.insert(axis, 0)
        state.append(center + np.array(pos))
    return np.array(state, dtype=np.float32)


def load_tool_repr(args):
    tool_full_repr_dict = {}
    for tool_name, tool_geom_list in args.tool_geom_mapping.items():
        tool_repr_points_list = []
        for i in range(len(tool_geom_list)):
            tool_repr_points_path = os.path.join(args.tool_repr_path, f'{tool_geom_list[i]}_points.npy')
            if os.path.exists(tool_repr_points_path):
                tool_repr_points_list.append(np.load(tool_repr_points_path, allow_pickle=True))
            else:
                tool_mesh = o3d.io.read_triangle_mesh(os.path.join(args.tool_repr_path, f'{tool_geom_list[i]}_repr.stl'))
                tool_surface_dense = o3d.geometry.TriangleMesh.sample_points_uniformly(tool_mesh, 100000, seed=0)

                tool_surface = tool_surface_dense.voxel_down_sample(voxel_size=0.007)                
                tool_repr_points = np.asarray(tool_surface.points)

                tool_repr_points_list.append(tool_repr_points)
                with open(tool_repr_points_path, 'wb') as f:
                    np.save(f, tool_repr_points)

        tool_full_repr_dict[tool_name] = tool_repr_points_list

    return tool_full_repr_dict


def get_normals(points, pkg='numpy'):
    B = points.shape[0]
    search_param = o3d.geometry.KDTreeSearchParamHybrid(radius=0.01, max_nn=8)
    if pkg == 'torch':
        points = points.detach().cpu().numpy().astype(np.float64)

    tool_normals_list = []
    for b in range(B):
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(points[b])
        pcd.estimate_normals(search_param, fast_normal_computation=True)
        pcd.orient_normals_towards_camera_location(pcd.get_center())
        normals = np.negative(np.asarray(pcd.normals))
        # pcd.normals = o3d.utility.Vector3dVector(normals)
        # visualize_o3d([pcd], show_normal=True)
        if pkg == 'torch':
            normals = torch.tensor(normals, dtype=torch.float32)

        tool_normals_list.append(normals)

    if pkg == 'torch':
        return torch.stack(tool_normals_list)
    else:
        return np.stack(tool_normals_list)


def get_tool_repr(args, fingertip_T_batch, pkg='numpy'):
    tool_dim_list = args.tool_dim
    if args.full_repr:
        tool_repr_list = []
        for i in range(fingertip_T_batch.shape[0]):
            if pkg == 'numpy':
                tool_repr_part = (fingertip_T_batch[i, :3, :3] @ np.array(args.tool_full_repr_dict[args.env][i]).T).T + \
                    np.tile(fingertip_T_batch[i, :3, 3], (tool_dim_list[i], 1))
            else:
                tool_full_repr = torch.tensor(args.tool_full_repr_dict[args.env][i], dtype=torch.float32, device=fingertip_T_batch.device)
                tool_repr_part = (fingertip_T_batch[i, :3, :3] @ tool_full_repr.T).T  + torch.tile(fingertip_T_batch[i, :3, 3], (tool_dim_list[i], 1))
            tool_repr_list.append(tool_repr_part)

        if pkg == 'numpy':
            tool_repr = np.concatenate(tool_repr_list)
        if pkg == 'torch':
            tool_repr = torch.cat(tool_repr_list)
    else:
        raise NotImplementedError

    return tool_repr



def add_tool_positions(args, state_seq, init_pose_seq, act_seq):
    state_seq_new = []
    for i in range(act_seq.shape[0]):
        tool_pos_list = []
        tool_start = 0
        for j in range(len(args.tool_dim)):
            tool_dim = args.tool_dim[j]
            tool_pos = init_pose_seq[i, tool_start:tool_start+tool_dim]
            tool_pos_list.append(tool_pos)
            tool_start += tool_dim

        for j in range(act_seq.shape[1]):
            step = i * act_seq.shape[1] + j
            for k in range(len(args.tool_dim)):
                tool_pos_list[k] += np.tile(act_seq[i, j, 6*k:6*k+3], (tool_pos_list[k].shape[0], 1))

            state_new = np.concatenate([state_seq[step], *tool_pos_list])
            state_seq_new.append(state_new)
    
    return np.array(state_seq_new)

def get_act_seq_from_state_seq(args, state_shape_seq):

    act_seq = []
    actions = []
    
    # Iterate over each state in the state sequence
    for i in range(1, state_shape_seq.shape[0]):
        action = []
        tool_start_idx = sum(args.n_particles_type) - args.n_particles_type[-1]
        
        # Iterate over each tool in the state
        for j in range(len(args.tool_dim)):
            tool_dim = args.tool_dim[j]
            state_diff = state_shape_seq[i, tool_start_idx] - state_shape_seq[i - 1, tool_start_idx] # TODO: check whether to use first or use average
            action.extend([state_diff, np.zeros(3)])
            tool_start_idx += tool_dim

        actions.append(np.concatenate(action))

    act_seq.append(actions)

    return np.array(act_seq)


def get_normals_from_state(args, state, visualize=False):
    state_normals_list = []

    dough_points = state[:args.n_particles]
    dough_normals = get_normals(dough_points[None])[0]
    state_normals_list.append(dough_normals)
    
    dough_pcd = o3d.geometry.PointCloud()
    dough_pcd.points = o3d.utility.Vector3dVector(dough_points)
    dough_pcd.normals = o3d.utility.Vector3dVector(dough_normals)

    state_normals_list.append(args.floor_normals)
    
    floor_pcd = o3d.geometry.PointCloud()
    floor_points = state[args.n_particles: args.n_particles+args.floor_dim]
    floor_pcd.points = o3d.utility.Vector3dVector(floor_points)
    floor_pcd.normals = o3d.utility.Vector3dVector(args.floor_normals)

    tool_start = args.n_particles + args.floor_dim
    tool_pcd_list = []
    for k in range(len(args.tool_dim)):
        tool_dim = args.tool_dim[k]
        tool_points = state[tool_start:tool_start+tool_dim]
        tool_normals = get_normals(tool_points[None])[0]
        state_normals_list.append(tool_normals)

        tool_pcd = o3d.geometry.PointCloud()
        tool_pcd.points = o3d.utility.Vector3dVector(tool_points)
        tool_pcd.normals = o3d.utility.Vector3dVector(tool_normals)
        tool_pcd_list.append(tool_pcd)
        
        tool_start += tool_dim

    # import pdb; pdb.set_trace()
    if visualize:
        o3d.visualization.draw_geometries([dough_pcd, floor_pcd, *tool_pcd_list], point_show_normal=True)

    return np.concatenate(state_normals_list, axis=0)


def inplace_relu(m):
    classname = m.__class__.__name__
    if classname.find('ReLU') != -1:
        m.inplace=True


# def read_ros_bag(bag_path):
#     pcd_msgs = []
#     while True:
#         try:
#             bag = rosbag.Bag(bag_path) # allow_unindexed=True
#             break
#         except rosbag.bag.ROSBagUnindexedException:
#             print('Reindex the rosbag file:')
#             os.system(f"rosbag reindex {bag_path}")
#             bag_orig_path = os.path.join(os.path.dirname(bag_path), 'pcd.orig.bag') 
#             os.system(f"rm {bag_orig_path}")
#         except rosbag.bag.ROSBagException:
#             continue

#     for topic, msg, t in bag.read_messages(
#         topics=['/cam1/depth/color/points', '/cam2/depth/color/points', '/cam3/depth/color/points', '/cam4/depth/color/points', 
#         '/ee_pose', '/gripper_width']
#     ):
#         if 'cam' in topic:
#             pcd_msgs.append(msg)

#     bag.close()

#     return pcd_msgs


def convert_range(value, old_min=-1, old_max=1, new_min=0.25, new_max=0.75):
    """
    Convert a value from one range to another.

    Parameters:
        value (float): The value to convert.
        old_min (float): The minimum value of the original range.
        old_max (float): The maximum value of the original range.
        new_min (float): The minimum value of the new range.
        new_max (float): The maximum value of the new range.

    Returns:
        new_value (float): The value converted to the new range.
    """
    old_range = old_max - old_min
    new_range = new_max - new_min
    new_value = (((value - old_min) * new_range) / old_range) + new_min
    return new_value


def point_within_box(point, box_min, box_max):
    """
    Check if a point is within a box defined by the minimum and maximum coordinates.

    Args:
        point (numpy.ndarray): The point to check, in the format [x, y, z].
        box_min (numpy.ndarray): The minimum coordinates of the box, in the format [x_min, y_min, z_min].
        box_max (numpy.ndarray): The maximum coordinates of the box, in the format [x_max, y_max, z_max].

    Returns:
        bool: True if the point is within the box, False otherwise.
    """
    return np.all(point >= box_min) and np.all(point <= box_max)


def copy_variable(x):
    if isinstance(x, np.ndarray):
        return copy.deepcopy(x)
    elif isinstance(x, torch.Tensor):
        return x.clone()
    elif isinstance(x, float):
        return x.copy()
    else:
        raise ValueError('Unsupported variable type')
    

def concat_tensors_or_arrays(a, dim):
    a1, a2 = a

    if isinstance(a1, torch.Tensor) and isinstance(a2, torch.Tensor):
        # Ensure a1 has the same number of dimensions as a2
        a2_expanded = a2.view(*[1 for _ in range(len(a1.shape) - 1)], dim)
        a2_expanded = a2_expanded.expand_as(a1[..., :1])
        return torch.cat((a1, a2_expanded), dim=dim)
    elif isinstance(a1, np.ndarray) and isinstance(a2, np.ndarray):
        # Ensure z has the same number of dimensions as a1

        a2_expanded = a2.reshape(*[1 for _ in range(len(a1.shape) - 1)], dim)
        a2_expanded = np.broadcast_to(a2_expanded, a1[..., :1].shape)
        return np.concatenate((a1, a2_expanded), axis=dim)
    else:
        raise ValueError("a1 and a2 must both be either torch.Tensor or numpy.ndarray")


def clamp(x, min_val, max_val):
    if isinstance(x, torch.Tensor):
        return torch.clamp(x, min_val, max_val)
    else:
        return np.clip(x, min_val, max_val)

    
    
def broadcast_pos_to_quat_shape(pos, quat):
    if isinstance(pos, np.ndarray) and isinstance(quat, np.ndarray):
        quat_shape = quat.shape
        if pos.ndim == 1:
            return np.broadcast_to(pos, (*quat_shape[:-1], 3))
        elif pos.ndim == 3:
            return pos
        else:
            raise ValueError("Invalid pos dimensions")
    elif isinstance(pos, torch.Tensor) and isinstance(quat, torch.Tensor):
        quat_shape = quat.shape
        if pos.ndim == 1:
            return torch.broadcast_to(pos, (*quat_shape[:-1], 3))
        elif pos.ndim == 3:
            return pos
        else:
            raise ValueError("Invalid pos dimensions")
    else:
        raise ValueError("Both pos and quat must be either torch.Tensor or np.ndarray")


def broadcast_quat_to_pos_shape(quat, pos):
    if isinstance(pos, np.ndarray) and isinstance(quat, np.ndarray):
        pos_shape = pos.shape
        if quat.ndim == 1:
            return np.broadcast_to(quat, (*pos_shape[:-1], 4))
        elif quat.ndim == 3:
            return quat

    elif isinstance(pos, torch.Tensor) and isinstance(quat, torch.Tensor):
        pos_shape = pos.shape
        if quat.ndim == 1:
            return torch.broadcast_to(quat, (*pos_shape[:-1], 4))
        elif quat.ndim == 3:
            return quat
    else:
        raise ValueError("Both pos and quat must be either torch.Tensor or np.ndarray")
