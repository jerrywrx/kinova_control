...
def preprocess_raw_pcd(args, obs, static_pcd, env_config, camera_names=['agentview'], rm_stats_outliers=1, visualize=False, env=None):
    ...
    if args.rescontruct_from_pose:
        ...
        ################################ pcd for gripper ################################
        gripper_width = robot_utils.get_gripper_width(args, obs)
...
# def gen_rollouts(args, n_rollouts, max_skill_calls, rollout_dir, thread_idx):
def gen_rollouts(rollout_infos):
    ...
    # env_dp = EnvRobosuite(env_name='Stow'+args.skill_name.capitalize(),
    #                       render=False,
    ...
    # while i < n_rollouts:
        ...
        # args = get_particle_config(args)
        # store_json(args.env_kwargs, f"{rollout_dir}/{rollout_idx:03d}/env_config.json")

        # pcd_dict, dis_wall = preprocess_raw_pcd(
        #     args, env.camera_names, obs, crop_bound, env=env)
        # # store_data(['num_objects'], len(pcd_dict), f"{rollout_dir}/stats.h5")
        # save_data_step(args, pcd_dict, dis_wall, env.camera_names,
        #                obs, rollout_dir, rollout_idx, 0, env)
        ...
        for skill_idx in np.arange(max_skill_calls):
            ...
            # with open(f"{rollout_dir}/{rollout_idx:03d}/{skill_idx:03d}_hl_param.npy", 'wb') as f:
            #     np.save(f, hl_param)
            ...
            while True:
                ...
                if verbose:
                    video_writer.append_data(next_obs[f"cam_1_image"])
                ...
                if expected_cur_pose != None:                    
                    ...
                    # cur_pose_state_idx=skill_controller._cur_skill.__class__.STATES.index(skill_controller._cur_skill._state)-1
                    # cur_pose_state = skill_controller._cur_skill.__class__.STATES[cur_pose_state_idx]
                    # expected_cur_pose = skill_controller._cur_skill.subgoal_poses[cur_pose_state]
            ...
            if invalid_episode:
                invalid_episode = False
                break
            # expected_final_st_idx = skill_controller._cur_skill.__class__.STATES.index('FINISHED') - 2
            # expected_final_st = skill_controller._cur_skill.__class__.STATES[expected_final_st_idx]
            # expected_cur_pose =  skill_controller._cur_skill.subgoal_poses[expected_final_st]
            # tool_repr = keypose_to_tool_repr(args, expected_cur_pose, obs)
            # h5_data = preprocess_raw_pcd(
            #     args, obs, static_pcd, env_config, env.camera_names, env=env)
            # h5_data.append(tool_repr)
...
def main():
    ...
    if args.gen_rollout:
        ...
        # gen_rollouts(args, n_rollouts=n_rollouts//args.num_workers, max_skill_calls=max_skill_calls,
        #              rollout_dir=os.path.join(rollout_dir, time_now))
    ...
    #     "/home/haonan/wd_drive/robocraft3d", 'raw_data', args.tool_type)
