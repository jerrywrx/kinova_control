...
class SingleArm(Manipulator):
    ...
    def control(self, action, policy_step=False):
        ...
        if policy_step:
            ...
            # If we're using a prior orientation, we need to compute the orientation to absolute values
            if self.controller.use_prior_orientation:
                set_ori = self.controller.get_prior_mat_from_prob(arm_action[3:])
                # set_ori = T.euler2mat(euler)

            self.controller.set_goal(
                arm_action, 
                set_ori=set_ori, # if set, will override oriention in arm_action  
            )
