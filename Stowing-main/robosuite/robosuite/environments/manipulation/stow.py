...
from robosuite.models.arenas import StowingArena
...
class Stow(SingleArmEnv):
    ...
    def _load_model(self):
        ...
        mujoco_arena = StowingArena(
            shelf_pos=self.shelf_pos, table_full_size=self.table_full_size, table_friction=self.table_friction
        )
        ...
        self.cubeA = BoxObject(
            name="cubeA",
            size_min=[0.1, 0.15, 0.04],
            size_max=[0.1, 0.2, 0.03],
            rgba=[1, 0, 0, 1],
            friction=1,
            density=43,
        )
        ...
        self.num_obj_on_shelf = 2 # random.randint(2, 4)
        self.shelf_objects = []
        for i in range(self.num_obj_on_shelf):
            self.shelf_objects.append(BoxObject(
                name="cubeB_"+str(i),
                size_min=[0.1, 0.03, 0.15],
                size_max=[0.1, 0.04, 0.25],
                rgba=[0, 1, 0, 1],
                friction=0.1,
                density=63,
            ))
        ...
        self.objects = [self.cubeA] + self.shelf_objects
        ...
        self.placement_initializer.append_sampler(
            sampler=UniformRandomSampler(
                ...
                x_range=[-0.01, 0.01],
                y_range=[-0.0, 0.19], # based on the size of shelf, from stowing_arena.xml
                rotation=(-np.pi/9, -np.pi/9),
                rotation_axis="x",
