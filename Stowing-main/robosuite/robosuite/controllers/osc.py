...
class OperationalSpaceController(Controller):
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        self.use_prior_orientation = use_prior_orientation

        # Control dimension
        self.control_dim = 6 if self.use_ori else 3
        self.name_suffix = "POSE_STOW" if self.use_ori and self.use_prior_orientation else "POSE" if self.use_ori else "POSITION"  
    ...
    def get_prior_mat_from_prob(self, ori_prob):
        ori_idx = np.argmax(ori_prob)
                
        # convert euler angle to rotation matrix
        push_euler_error = np.array([0, 0 , -np.pi/2])
        grasp_euler_error = np.array([np.pi/2 ,-np.pi/2 , 0])
        insert_euler_error =  np.array([0, -np.pi/2 , 0])
        euler_errors = [push_euler_error, grasp_euler_error, insert_euler_error]
        
        euler_error = euler_errors[ori_idx] 
        rotation_mat_error = T.euler2mat(euler_error)
        # rotation_mat_error = T.rotation_matrix(angle=np.pi/2, direction=[0.0, 1.0, 0.0])[:3, :3]
        goal_orientation = np.dot(rotation_mat_error, self.initial_ee_ori_mat)
