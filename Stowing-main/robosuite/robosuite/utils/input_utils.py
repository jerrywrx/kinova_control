...
def input2action(device, robot, active_arm="right", env_configuration=None):
    ...
    rotation_prob = state.get("rotation_prob", np.array([1, 0 ,0]))
    ...
    elif controller.name == "OSC_POSITION" or "OSC_POSE_STOW":
        dpos = dpos * 75 if isinstance(device, Keyboard) else dpos * 125
    else:
    ...
    elif controller.name == "OSC_POSE_STOW":
        action = np.concatenate([dpos, rotation_prob, [grasp] * gripper_dof])
    else:
