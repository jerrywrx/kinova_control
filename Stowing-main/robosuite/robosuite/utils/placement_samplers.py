...
class UniformRandomSampler(ObjectPositionSampler):
    ...
    def __init__(
        ...
        reference_pos=(0, 0, 0),
        z_offset=0.0,
        rect_sample=False,# whether to sample by generating multiple non-overlapping rectangles
        shelf_overlap_check=False,# whether to check if the object is parallel to the other objects on the shelf
    ):
        ...
        self.rect_sample = rect_sample
        self.shelf_overlap_check = shelf_overlap_check
    ...
    def sample(self, fixtures=None, reference=None, on_top=True):
        ...
        if self.rect_sample:
            horizontal_radius = max([obj.size[1] for obj in self.mujoco_objects])
            rects = self._sample_rects(horizontal_radius)
            ...
            for obj in self.mujoco_objects:
                ...
                success = False
                for i in range(5000): # try 5000 times to place the object
                    if not rects:
                        rects = self._sample_rects(horizontal_radius) 
                    object_xy = rects.pop(random.randrange(len(rects)))
                    object_x = (object_xy[0]+object_xy[1])/2 + base_offset[0]
                    object_y = (object_xy[2]+object_xy[3])/2 + base_offset[1]
                    ...
                    if self.ensure_valid_placement:
                        for (x, y, z), _, other_obj in placed_objects.values():
                            if (
                                ...
                                break
                    if self.shelf_overlap_check:
                        for (x, y, z), _, other_obj in placed_objects.values():
                            if (
                                np.absolute((object_y - y))
                                <= (other_obj.size[1] + obj.size[1])
                            ) and (object_z - z <= other_obj.top_offset[-1] - bottom_offset[-1]):
                                location_valid = False
                                break
    ...
    def _quadsect(self, xy_range, horizontal_radius, factor=3):
        ...
        """
        x_range, y_range = xy_range[:2], xy_range[2:]
        ...
        if self.ensure_object_boundary_in_range:
            delta_x = plus_or_minus(random.uniform(0, x_length/2 - horizontal_radius))
            delta_y = plus_or_minus(random.uniform(0, y_length/2 - horizontal_radius))
        else:
            delta_x = plus_or_minus(random.uniform(0, x_length // factor))
            delta_y = plus_or_minus(random.uniform(0, y_length // factor))
        
        interior_x = (x_range[1] + x_range[0])/2 + delta_x
        interior_y = (y_range[1] + y_range[0])/2 + delta_y

        # return four rectangles
        return [(x_range[0], interior_x, y_range[0], interior_y),
                (interior_x, x_range[1], y_range[0], interior_y),
                (x_range[0], interior_x, interior_y, y_range[1]),
                (interior_x, x_range[1], interior_y, y_range[1])]
        
    def _sample_rects(self, object_horizontal_radius):
        """ Sample a list of rectangles, each of which contains a single object.
            The rectangles are non-overlapping and cover the entire xy range.    
        """
        whole_xy_range = (self.x_range[0], self.x_range[1], self.y_range[0], self.y_range[1])
        rects = [whole_xy_range]   # seed output list
        while len(rects) <= len(self.mujoco_objects):
            rects = [subrect for rect in rects
                                for subrect in self._quadsect(rect, object_horizontal_radius)]
        # sample = random.sample(rects, len(self.mujoco_objects))  # select the desired number
        return rects
