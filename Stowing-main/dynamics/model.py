...
class DynamicsPredictor(nn.Module):
    def __init__(self, args, residual=False):
        ...
        self.quat_offset = torch.tensor([1, 0, 0, 0], device=self.device) # scalar first
        ...
        # ParticleEncoder
        particle_input_dim = args.attr_dim + args.max_n_instance + 1 + (self.n_his*self.time_gap+1) * 3 * 2 + (self.state_dim -3) + self.mem_dim
        self.particle_encoder = Encoder(particle_input_dim, nf_particle, nf_effect)

        # RelationEncoder
        # relation_input_dim = particle_input_dim * 2 + 1 + (self.n_his*self.time_gap+1) * self.state_dim
        relation_input_dim = particle_input_dim * 2 + 1 
    ...
    def forward(self, inputs):
        ...
        # action_history: B x n_his x N x state_dim
        ...
        # n_p: the number of particles that need to be predicted (moving particles)
        # n_s: the number of particles of other objects (static and tools)
        # n_his: history length
        # state_dim: 3
        B, N = attrs.size(0), attrs.size(1)
        # n_p = p_instance.size(1)
        n_p = args.n_particles_type[0]
        n_s = attrs.size(1) - n_p
        distance_dim = args.state_dim - 3
        
        # Rr_cur_t, Rs_cur_t: B x N x n_rel
        Rr_cur_t = Rr_cur.transpose(1, 2).contiguous()
        # Rs_cur_t = Rs_cur.transpose(1, 2).contiguous()
        
        # p_instance_t: B x n_instance x n_particle
        p_instance_t = p_instance.transpose(1, 2)

        tool_start_idx = args.n_particles_type[0] + args.n_particles_type[1]
        


        # state_norm (normalized): B x n_his x N x state_dim
        # [0, n_his - 1): state_residual
        state_res_norm = torch.zeros(B, self.n_his*self.time_gap, N, 3, device=self.device)

        state_res_norm[:, :, tool_start_idx:, :3] = batch_normalize(action_his[:, 1:, :, :3] - action_his[:, :-1, :, :3], mean_d, std_d)

        # [n_his - 1, n_his): the current position
        state_cur_norm = batch_normalize(state_cur[...,:3].unsqueeze(1), mean_p, std_p)
        if not args.use_distance:
            state_norm = torch.cat([state_res_norm, state_cur_norm], 1)
        else:
            distance_cur_norm = batch_normalize(state_cur[...,3:].unsqueeze(1), mean_d, std_d)
            state_norm = torch.cat([state_res_norm, state_cur_norm, distance_cur_norm], 1)
        # state_norm_t (normalized): B x n_particle x (n_his * state_dim)
        state_norm_t = state_norm.transpose(1, 2).contiguous().view(B, N, (self.n_his*self.time_gap+1)*3+distance_dim)

        # add offset to center-of-mass for rigids to attr
        # offset: B x N x (n_his * state_dim)
        offset = torch.zeros(B, N, (self.n_his*self.time_gap+1)*3, device=self.device)
        ...
        instance_center = p_instance_t[:,:,:n_p].bmm(state_norm_t[:, :n_p, :(self.n_his*self.time_gap+1)*3])
        instance_center /= torch.sum(p_instance, 1).unsqueeze(-1) + 1e-6

        # center_per_particle: B x n_p x (n_his * state_dim)
        # c is the center of the instance of the rigid body that the particle belongs to:  B x n_p x (n_his * state_dim) 
        # particle offset: B x n_p x (n_his * state_dim)
        center_per_particle = p_instance[:, :n_p].bmm(instance_center)
        c = (1 - p_rigid_per_particle[:,:n_p]) * state_norm_t[:, :n_p, :(self.n_his*self.time_gap+1)*3] + p_rigid_per_particle[:,:n_p] * center_per_particle
        # offset is the offset of the particle from the center of the rigid body
        offset[:, :n_p] = state_norm_t[:, :n_p, :(self.n_his*self.time_gap+1)*3] - c
        ...
        # state_norm_r: B x n_rel x 6
        # state_norm_s: B x n_rel x 6
        ...
        if args.auxiliary_gripper_loss:
            p_0 = state_cur[:, None, :, :3]
            p_0 = p_0.repeat(1, n_instance, 1, 1).view(B * n_instance, -1, 3)
        else:
        ...
        if args.auxiliary_gripper_loss:
            rigid_motion = (p_1 - p_0).view(B, n_instance, -1, 3)
        else:
        ...
        if args.rigid_motion:
            if args.auxiliary_gripper_loss:
                ...
                rigid_motion = torch.sum(p_instance_t[:, :, :n_p, None] * rigid_motion, 1)
            if args.attn:
                ...
                if args.auxiliary_gripper_loss:
                    motion_norm = rigid_motion 
                else:
                    motion_norm = rigid_motion + (1. - p_rigid_per_particle[:,:n_p])*non_rigid_motion
        else:
            # if args.attn:
            #     nrm_attn = self.nrm_attn_layer(particle_effect[:, :n_p].contiguous())
        ...
        if args.check_tool_touching:
            is_tool_touching = torch.count_nonzero(Rs_cur[:, :, tool_start_idx:], dim=(1, 2))
            pred_motion = (is_tool_touching > 0).int().view(-1, 1, 1) * batch_denormalize(motion_norm, mean_d, std_d)
        else:
            pred_motion = batch_denormalize(motion_norm, mean_d, std_d)

        if args.motion_bound > 0:
            pred_pos = state_cur[:, :n_p, :3] + torch.clamp(pred_motion, max=self.time_gap*args.motion_bound, min=-self.time_gap*args.motion_bound)
        else:
            pred_pos = state_cur[:, :n_p, :3] + pred_motion[:,:n_p,:3]
        if args.auxiliary_gripper_loss:
            pred_tool_pos = state_cur[:, tool_start_idx:, :3] + pred_motion[:,tool_start_idx:,:3]
        
        # pred_pos (unnormalized): B x n_p x state_dim
        # pred_motion_norm (normalized): B x n_p x state_dim
        if not args.auxiliary_gripper_loss:
            pred_tool_pos = None

        return pred_pos, pred_motion, nrm_attn, pred_tool_pos, pred_transformation
