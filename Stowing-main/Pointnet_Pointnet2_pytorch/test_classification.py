...
def test(model, loader, num_class=40, vote_num=1):
    ...
    for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
        # points.shape = torch.Size([24, 1024, 3])
        if not args.use_cpu:
        ...
        for _ in range(vote_num):
            # points.shape = torch.Size([24, 3, 1024])
...
def main(args):
    ...
    '''CREATE DIR'''
    experiment_dir = os.path.join(pathlib.Path(__file__).parent.resolve(), 'log/classification/' + args.log_dir) 
