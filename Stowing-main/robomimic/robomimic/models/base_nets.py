...
from robomimic.models.pointnet2_utils import PointNetSetAbstractionMsg,PointNetSetAbstraction,PointNetFeaturePropagation
import robomimic.models.pointnet2_provider as PointNet2Provider
...
================================================
"""
class pointnet2_cls_ssg_model(nn.Module):
    def __init__(self, num_class=40, normal_channel=True):
        super(pointnet2_cls_ssg_model, self).__init__()
...
class PointNet2Core(EncoderCore, nn.Module):
    def __init__(
        self,
        input_shape,
        feature_dimension=None,
        ...
        """
        Args:
            normal_channel (bool): whether to use normal channel
            flatten (bool): whether to flatten the visual feature
            feature_dimension (int): if not None, add a Linear layer to
                project output into a desired feature dimension
        """
        super(PointNet2Core, self).__init__(input_shape=input_shape)

        in_channel = 6 if normal_channel else 3
        self.normal_channel = normal_channel

        # load pointnet++ backbone with pretrained weights
        pointnet2_backbone = pointnet2_cls_ssg_model(normal_channel=normal_channel)
        weights_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), \
                                'pointnet2_log/classification/pointnet2_ssg_wo_normals/checkpoints/best_model.pth')
        checkpoint = torch.load(weights_path)
        pointnet2_backbone.load_state_dict(checkpoint['model_state_dict'])

        num_ftrs = pointnet2_backbone.fc3.in_features # or self.pointnet2_features[-3].out_features
        pointnet2_backbone.fc3 = nn.Linear(num_ftrs, feature_dimension)
        
        # freeze all layers except the last one
        for param in pointnet2_backbone.parameters():
            param.requires_grad = False
            
        for param in pointnet2_backbone.fc3.parameters():
            param.requires_grad = True
            
        self.pointnet2_features = pointnet2_backbone
...
class LowDimRandomizer(Randomizer):
    """
    Randomly add gussian noise to the input.
    """
    def __init__(
        self,
        input_shape,
        ...
        Args:
            input_shape (tuple, list): shape of input (not including batch dimension)
        """
        super(LowDimRandomizer, self).__init__()
    ...
    def forward_in(self, inputs):
        ...
        inputs = inputs + self.noise_std*torch.randn(inputs.shape, device=inputs.device)
        return inputs

    def forward_out(self, inputs):
        """
        do nothing to the output from the encoder
        """
        return inputs
    ...
    def __repr__(self):
        """Pretty print network."""
        header = '{}'.format(str(self.__class__.__name__))
        msg = header + "(input_shape={}, noise_std={})".format(
            self.input_shape, self.noise_std)
        return msg
...
class PointCloudRandomizer(Randomizer):
    ...
    def forward_in(self, inputs):
        """
        randomly drop points, scale points, and shift points
        """
        assert len(inputs.shape) >= 2 # must have at least (3, N) dimensions
        points = inputs.transpose(2, 1).cpu().data.numpy() # (B, N, 3)
        points = PointNet2Provider.random_point_dropout(points)
        points[:, :, 0:3] = PointNet2Provider.random_scale_point_cloud(points[:, :, 0:3])
        points[:, :, 0:3] = PointNet2Provider.shift_point_cloud(points[:, :, 0:3])
        points = torch.from_numpy(points).float().to(inputs.device) 
