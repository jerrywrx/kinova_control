...
class PointCloudModality(Modality):
    ...
    def _default_obs_processor(cls, obs):
        ...
        """
        # if the input has already been processed, return it
        if obs.shape[-2] == 3:
            return obs
        if not np.any(obs):
        ...
        if len(obs.shape) == 4: # input with batch dimension
            ...
            pcd = np.array([PointCloudModality._down_sample(single_pcd, num_samples) for single_pcd in pcd])
            
            if torch.is_tensor(pcd):
                pcd = pcd.transpose(2, 1)
            else:
                pcd = pcd.transpose([0, 2, 1])
        elif len(obs.shape) == 3: # input without batch dimension
            pcd = obs.reshape([-1, 3]) # out shape: (H*W, 3)
            pcd = PointCloudModality._down_sample(pcd, num_samples)
    ...
    @staticmethod
    def _down_sample(obs, num_samples):
        """
        ...
        Returns:
            processed_obs (np.array or torch.Tensor): processed point cloud
        """
        # Uses a regular voxel grid to create a uniformly downsampled point cloud from an input point cloud.
        # The voxel grid is defined by a voxel size.
        # All points lying within a voxel will be averaged to obtain a single point in the downsampled point cloud.
        # obs: (H*W, 3) -> out: (H*W, 3)
        # obs: (B, H*W, 3) -> out: (B, H*W, 3)
        assert obs.shape[-1] == 3, f"Point cloud data (xyz) should have last dimension 3, found {obs.shape[-1]}."
        if torch.is_tensor(obs):
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(obs.cpu().numpy())
        else:
            pcd = o3d.geometry.PointCloud()
        ...
        if torch.is_tensor(obs):
            pcd_down = torch.from_numpy(np.asarray(pcd_down.points)).to(obs.device)
        else:
