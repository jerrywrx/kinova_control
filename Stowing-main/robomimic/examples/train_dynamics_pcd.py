...
from robomimic.scripts.train_dymaics import train_dynamics
...
def robosuite_hyperparameters(config):
    ...
    # config.observation.modalities.obs.rgb = ['agentview_image']                # no image observations
    # config.observation.modalities.obs.depth = ['agentview_depth']                # no image observations
    config.observation.modalities.obs.pcd = ['agentview_xyz']                # no image observations
    ...
    # observation encoder for point clouds
    config.observation.encoder.pcd.core_class = "PointNet2Core"
    config.observation.encoder.pcd.core_kwargs.feature_dimension = 64
    config.observation.encoder.pcd.core_kwargs.normal_channel= False
    config.observation.encoder.pcd.obs_randomizer_class = None
    
    
    ################## randomizer config ##################
    # observation randomizer class - set to None to use no randomization, or 'LowDimRandomizer' to use low-dim randomization
    config.observation.encoder.low_dim.obs_randomizer_class = 'LowDimRandomizer'
    config.observation.encoder.low_dim.obs_randomizer_kwargs.noise_std = 0.01 #TODO: check this
    ...
    # observation randomizer class - set to None to use no randomization, or 'PointCloudRandomizer' 
    config.observation.encoder.pcd.obs_randomizer_class = 'PointCloudRandomizer'
    # kwargs for observation randomizers (for the CropRandomizer, this is size and number of crops)
    ########################################################
    ...
    config.algo.actor_layer_dims = (512, 512)  
...
def get_config(dataset_type="robosuite", dataset_path=None, output_dir=None, debug=False):
    ...
    config = config_factory(algo_name="dynamics_algo")

    ### Experiment Config ###
    config.experiment.name = f"{dataset_type}_dynamics_pcd"   # name of experiment used to make log files
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default='/home/haonan/Projects/Stowing/robosuite-1.2/robosuite/models/assets/demonstrations/1674532569_564803/image.hdf5',
    ...
    #     "--output",
    #     type=str,
    #     default='output/tmp_trained_models',
    ...
    # log_directory = os.path.join('log', timestr)
    log_directory = 'log'
    ...
    # run training
    train_dynamics(config, device=device)
